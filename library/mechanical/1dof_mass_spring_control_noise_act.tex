...
\begin{tikzpicture}
  ...
  % Noise
  \node[draw, add, minimum size=\addsize, left=\spacing of meas] (addn) {};
  \draw[->] (meas.center) node[above left]{$d$} -- (addn.east);
  \draw[<-] (addn.west) -- ++(-0.8*\spacing, 0) node[above right]{$n$};

  % Controller
  \node[draw, block, above=\spacing of m] (K) {$K$};
  \draw[->] (addn.north) |- (K.west) node[above left]{$d_m$};

  % Force injected and input noise
  \node[draw, add, minimum size=\addsize, right=\spacing of F] (addF) {};
  \draw[->] (K.east) -| (addF.north);
  \draw[->] (addF.west) -- (F.east) node[above right]{$F$};
  \draw[<-] (addF.east) -- ++(0.8*\spacing, 0) node[above left]{$i$};
\end{tikzpicture}
\end{document}

...
\begin{tikzpicture}
  ...
  \draw[actuator] ( 0.3*\massw, 0) -- (	0.3*\massw, \spaceh) node[midway, right=0.1](F){$\hat{F}$};
 
  % Displacements
  \draw[dashed] (0.5*\massw, 0) -- ++(0.2*\massw, 0);
  \draw[->] (0.6*\massw, 0) -- ++(0, 0.2*\spaceh) node[right]{$\hat{x_g}$};
  \draw[dashed] (0.5*\massw, \spaceh) -- ++(0.2*\massw, 0);
  \draw[->] (0.6*\massw, \spaceh) -- ++(0, 0.2*\spaceh) node[right]{$\hat{x}$};

  % Measurement
  \draw[dashed] (-0.5*\massw, 0) -- ++(-0.2*\massw, 0);
  \draw[dashed] (-0.5*\massw, \spaceh) -- ++(-0.2*\massw, 0);
  \draw[dashed, <->] (-0.6*\massw, 0) -- ++(0, \spaceh) node[midway, left](meas){$\hat{d}$};
\end{tikzpicture}
\end{document}